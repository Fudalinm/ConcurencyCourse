...
class Producer(name: String, buf: ActorRef) extends Actor {
  ...
  private def iter(): Unit = {
    if (repeat > 0) {
      ...
      println("P" + name + "(" + repeat + ") produkuje: " + product)
      buf ! Put(product)
      repeat -= 1
    } else {
      println("P" + name + " Koncze dzialanie!")
      buf ! ImDying
      context.stop(self)
    }
  }
...
class Consumer(name: String, buf: ActorRef) extends Actor {
  ...
  private def iter(): Unit = {
    if (repeat > 0) {
      ...
      println("C" + name + " Koncze dzialanie!")
      buf ! ImDying
      context.stop(self)
    }
  }
  ...
  def receive = LoggingReceive {
    ...
    case ConsumeDone(x) => {
      println("C" + name + "(" + (repeat + 1) + "): otrzymalem " + x)
      iter()
    }
  }
}
...
object ProdConsMain extends App {
  ...
  for (i: Int <- 1 to consNum) {
    ...
    println("Zaraz wystartuje konsumenta: " + i)
    c ! Init
  }
  ...
  for (i: Int <- 1 to prodNum) {
    ...
    println("Zaraz wystartuje producenta: " + i)
    p ! Init
  }
